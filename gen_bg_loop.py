...
def generate_bg_loop(image_path, prompt, output_filename=None, api_key=None, enhance=True, assets_dir=None):
    # 決定輸出資料夾，預設為 storybook/assets/
    out_dir = assets_dir if assets_dir else ASSETS_DIR
    os.makedirs(out_dir, exist_ok=True)
    filename = output_filename if output_filename else "page1_idle.mp4"
    ...
    if enhance:
        print("正在呼叫 Gemini 模型優化背景動畫提示詞...")
        system_instruction = (
            "You are an expert prompt engineer for AI video models like Veo. "
            "The user will provide an idea for a BACKGROUND animation video that involves a REFERENCE IMAGE. "
            "Your top priority is to expand the user's idea into a detailed, cinematic video prompt in English.\n"
            "CRITICAL: You MUST explicitly instruct that the video must end exactly as it started, returning the background to its original static pose (looping back to the original state).\n\n"
    ...
    if operation.response and operation.response.generated_videos:
        ...
        if video.video.video_bytes:
            ...
            with open(output_path, "wb") as f:
                f.write(video_bytes)
        print(f"✅ 背景循環影片儲存成功：{output_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="生成首尾相同的背景循環影片 (Idle)")
    parser.add_argument("--image", required=True, help="輸入的圖片路徑")
    parser.add_argument("--prompt", required=True, help="背景動畫提示詞")
    parser.add_argument("--output", default="page1_idle.mp4", help="輸出的檔名 (預設: page1_idle.mp4)")
    ...
    generate_bg_loop(args.image, args.prompt, args.output, args.api_key, args.enhance, args.assets_dir)
